...
func testUploadClient(serverURL string) *upload.UploadClient {
	return upload.NewTestUploadClient(serverURL)
}
...
func TestUploadPendingArchiveSuccessUpdatesState(t *testing.T) {
	...
	if !runner.uploadPendingArchive(job, s, settings) {
...
func TestRunnerRetryAndPendingHelpers(t *testing.T) {
	...
	s := &state.JobState{PendingFingerprint: "fp", NextRetryAt: utcAfterSeconds(60)}
	if got := runner.checkRetry(job, s); got != "waiting" {
	...
	s = &state.JobState{PendingArchive: archivePath, PendingFingerprint: "fp", NextRetryAt: utcAfterSeconds(-1)}
	...
	if s.PendingArchive != "" {
		t.Fatalf("missing pending should be cleared: %+v", s)
	}

	runner.setActivePhase(job, s, "compressing", 150)
	got := runner.StateStore.Get(job.RootPath)
	if got.ActivePhase != "compressing" || got.ActivePhasePercent != 100 {
		t.Fatalf("setActivePhase clamp = %+v", got)
	}
	runner.setActivePhase(job, s, "scanning", -10)
	got = runner.StateStore.Get(job.RootPath)
	...
	s.PendingTimestamp = "ts"
	s.UploadID = "u"
	s.UploadOffset = 3
	s.CurrentChunkSizeBytes = ptrInt64(2)
	runner.clearPendingArchive(s)
	if s.PendingArchive != "" || s.PendingArchiveSize != nil || s.UploadID != "" {
		t.Fatalf("clearPendingArchive = %+v", s)
	}
...
func TestPrepareArchiveLockedPaths(t *testing.T) {
	...
	if !ready || s.PendingArchive == "" || s.PendingFingerprint == "" || s.PendingArchiveSHA256 == "" {
		t.Fatalf("archive state = %+v", s)
	}
...
func TestProcessJobLockedAndForceSendValidation(t *testing.T) {
	...
	st := runner.StateStore.Get(jobRoot)
	if st.LastStatus != "success" {
		t.Fatalf("force state = %+v", st)
	}
}
...
func TestRunnerMiscHelpers(t *testing.T) {
	...
	ctx := runner.hookContext(job, s, settings)
	if ctx["edge_id"] != "edge-1" || ctx["job_name"] != "job" || ctx["upload_offset"] != int64(42) {
		t.Fatalf("hook context = %+v", ctx)
	}

	retryAfter := 7
	if got := runner.retryDelaySeconds(3, &upload.UploadFailure{RetryAfterSeconds: &retryAfter}); got != retryAfter {
...
func TestCleanupStaleArchives(t *testing.T) {
	...
	if err := runner.StateStore.Set("job", state.JobState{PendingArchive: keep}); err != nil {
