...
func NewUploadClient(cfg *config.Settings, encKey []byte, certs *certificates.CertManager) *UploadClient {
...
// SetHTTPClient replaces the underlying HTTP client (used in tests).
func (c *UploadClient) SetHTTPClient(client *http.Client) {
	c.http = client
}

// NewTestUploadClient creates a minimal UploadClient for testing.
func NewTestUploadClient(serverURL string) *UploadClient {
	return &UploadClient{
