...
class RecoveryService:
    ...
    def recover_latest(self, job: JobDefinition) -> dict[str, object]:
        ...
        try:
            ...
            if exc.status_code == 404:
                recovery_error = RecoveryError("no snapshots found on Central", status_code=404)
            elif exc.category == "unauthorized":
                recovery_error = RecoveryError(
                    "Central rejected the recovery request; check the shared auth token",
                    status_code=502,
                )
            elif exc.category in {"network", "server", "rate_limited", "circuit_open"}:
                recovery_error = RecoveryError(str(exc), status_code=502)
            else:
                recovery_error = RecoveryError(str(exc), status_code=400)
            self._mark_failure(job, state, recovery_error.message)
            self.logger.error("recovery_failed job_name=%s path=%s detail=%s", job.job_name, job.root_path, recovery_error.message)
            raise recovery_error from exc
        except RecoveryError as exc:
            self._mark_failure(job, state, exc.message)
            self.logger.error("recovery_failed job_name=%s path=%s detail=%s", job.job_name, job.root_path, exc.message)
            raise
        except Exception as exc:
            self._mark_failure(job, state, str(exc))
    ...
    def _temp_path(self, suffix: str) -> Path:
        ...
        with tempfile.NamedTemporaryFile(dir=self.settings.spool_dir, delete=False, suffix=suffix) as handle:
