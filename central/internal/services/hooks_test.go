...
func TestIsUTF8_Valid(t *testing.T) {
	if !isUTF8([]byte("hello world")) {
		t.Error("ASCII should be valid UTF-8")
	}
	if !isUTF8([]byte("héllo wörld")) {
		t.Error("UTF-8 multi-byte should be valid")
	}
}

func TestIsUTF8_Invalid(t *testing.T) {
	if isUTF8([]byte{0xff, 0xfe, 0x00}) {
		t.Error("invalid bytes should not be UTF-8")
	}
}
